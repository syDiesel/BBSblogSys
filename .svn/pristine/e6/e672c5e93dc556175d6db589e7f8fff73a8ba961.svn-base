...
<title>${log.subject }-${log.blog.blogName }-Webname</title>
...
<script type="text/javascript" src="<%=basePath%>js/blog/blogMarks.js"></script>
...
<script type="text/javascript"
	src="<%=basePath%>js/Forum/blogToForum.js"></script>
				...
				<div class="tdtwContent" id="tdtwInfo">
					...
					<ul>
						...
						<c:forEach items="${question }" var="item" end="4">
							<li><a href="<%=basePath%>web/QandA/QuestionFind/${item.id}">${item.qSubject
									}</a>
							</li>
						</c:forEach>
					</ul>
				...
				<div class="tdtwContent" style="display: none" id="tdhdInfo">
					...
					<ul>
						...
						<c:forEach items="${answer }" var="item" end="4">
							<li><a
								href="<%=basePath%>web/QandA/QuestionFind/${item.question.id}">${item.question.qSubject}
									}</a></li>
						</c:forEach>
				...
				<div class="hybqContent">
					<table class="hybqInfo" border="0" width="100%" cellspacing="0"
						...
						<c:forEach items="${activeLabels }" var="item">
							...
							<tr>
								<td><span class="rank<%=rank%>"><%=rankCount%></span>
								</td>
								<td><a>${item.label.labelName }</a>
								</td>
								<td>${item.label.board.boardName }<%-- ${item.labelDesc } --%>
		...
		<div class="rightPart">
			<div class="articleInfo">
				...
				<div class="articleInfo_label">
					<div class="postDate">
						issues on <span>${log.blogTime }</span>
					</div>
					<div class="fenlei">
						Tag： <span> <c:forEach items="${blogLabels}" var="item">

								<a
									href="<%=basePath%>search.do?searchMethod=0&search=${item.label.labelName }">${item.label.labelName
									}</a>；
	            	</c:forEach> </span>
					...
					<div class="readCount">
						<img src="<%=basePath%>images/Forums/new/visitcount.png"
							width="16px" height="12px"> <em>${log.visited }</em>
					</div>


					<div class="postBtn">
						<c:if test="${owner==1 }">
							<a href="<%=basePath %>web/Blog/deleteLog_${log.id }"
								onclick="return confirm('Unable to retrieve once deleted.')"><span>Delete</span>
							</a>
						</c:if>
						<a href="javascript:void(0)" onclick="Collect()" id="collect">
							<c:if test="${isCollect==1 }">
								<span>Favorited
								</span>
							</c:if> <c:if test="${isCollect==0 }">
								<span>Favorite
								</span>
							</c:if> </a>
				...
				<div class="articleInfo_Content" oncopy="return false;" oncut="return false;">${log.blogContent }</div>
				<c:if test="${log.blogType==2 }">
					<div class="articleInfo_from">Article Source：${log.wzcc }</div>
				</c:if>
				</br> </br> </br> </br>
				<!--pingjia-->
				<div class="article_pingjia" >
				<div style="height:150px;">
				<div class="article_pingjia_tip" id="article_pingjia_tip">
				<span class="tip_font">
				Please comment on this article after reading
				</span>
				</div>
				</div>
					<input type="hidden" id="logId" value="${log.id }" /> <a
						onclick="pingjia(this)" id="btnGood"><div class="haoping">
							<span class="pingAlt">Good</span> <span class="haopingCount"
								id="btnGoodCount">${log.up }</span>
						</div> </a> <a onclick="pingjia(this)" id="btnMid"><div
							class="zhongping">
							<span class="pingAlt">SO SO</span><span class="zhongpingCount"
								id="btnMidCount">${log.normal }</span>

						</div> </a> <a onclick="pingjia(this)" id="btnBad">
						<div class="chaping">
							<span class="pingAlt">Sorry</span><span class="chapingCount"
								id="btnBadCount">${log.down }</span>
						</div> </a>
				</div>
			...
			<c:if test="${log.sfzz==1 }">
				<div class="repost">
					<div class="toForum">
						<form action="<%=basePath%>fromBlog${log.id}.do" method="post"
							id="toForumForm"></form>
						<input type="hidden" id="logId" value="${log.id}" name="blogId">
						<input type="hidden" id="basePath" value="<%=basePath%>">
						<a href="javascript:void(0);" id="blogToForum"><h3>Reprint to forum</h3>
						</a>
					</div>
				</div>
			</c:if>
...
<script>
	...
	setTimeout("article_pingjia_tip.style.display='none'",10000);
</script>
</html>
