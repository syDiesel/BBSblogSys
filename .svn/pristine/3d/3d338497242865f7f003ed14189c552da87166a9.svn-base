...
<script type="text/javascript">
       ...
       function changeStatus(){
        var  changestatus=document.getElementById("status").value; 
        var board_id=document.getElementById("masterBoardId").value; 
        
        window.location.href="setVerify_"+board_id+"?status="+changestatus+"&record=1";   
         }
...
<div class="floor1" style="background:#fff;">
     ...
     <div class="manageForum">
          <div class="boardIndex" >
              <span style="font:18px;float:left;margin-left:10px;margin-top:7px;">Approve:
              <c:if test="${board.isVerify.equals('1')}">Start</c:if>
              <c:if test="${board.isVerify.equals('0')}">Close</c:if>        
              </span>
          ...
          <div class="widget">
             ...
             <tr ><td>Title</td>
              ...
              <td>&nbsp;</td></tr>
                       ...
                       <tr>
                       <td><a href="toTopics?post_id=${listForum.get(size-1).id}">${listForum.get(size-1).subject}</a></td>
                       <td>
                       <a href="<%=basePath%>web/Blog/${listForum.get(size-1).userInfo.nickName}">
                       ${listForum.get(size-1).userInfo.nickName}
                       </a></td>
                       <td >${listForum.get(size-1).postDate}</td> 
                       <td><a href="passForum${listForum.get(size-1).id}.do?msg=1&record=${record}#mainbox">Pass</a>     
                       |   <a href="passForum${listForum.get(size-1).id}.do?msg=0&record=${record}#mainbox">Not pass</a></td>
                       </tr>
                       </c:forEach>
          ...
          <div class="fenye">
          	    <!-- 此处设置分页 -->
						 ...
						 <a href="setVerify_${masterBoardId}?&record=${record-1}">Page up</a> 
						</c:if>
						...
						<c:forEach begin="1" end="${allPage}" step="1" var="i">
						      ...
						      <a href="setVerify_${masterBoardId}?record=${i}">
						...
						<c:forEach begin="${record-5}" end="${record+4}" step="1" var="i">
						 ...
						 <a href="setVerify_${masterBoardId}?record=${record+1}">Page down</a>
                         </c:if>
