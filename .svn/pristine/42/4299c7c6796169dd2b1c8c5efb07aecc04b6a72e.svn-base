  ...
  <div class="login_div" id="login_div">
	...
	<div class="blog_main">
		<div class="blog_title">
			<h3>personal forum</h3>
			<c:if test="${blogUser.id eq userInfo.id}">
			<button class="submitButton" onclick="javascrtpt:window.location.href='<%=basePath%>addPost'"
			style="float: right;margin-top:-22px;margin-right:30px;">issue topic</button>
			</c:if>
		</div>
		<br>
			...
			<div class="interest_blog_Info_left ">
				<div class="blog_content">
					<table class="interest_blog_Info_left_Info" border="0"
						...
						<c:if test="${!empty forumByTime }">
							<c:forEach items="${forumByTime}" var="item" end="14">
								<tr>
				...
				<c:if test="${!empty forumByTime }">
					<div class="page">
						<ul>
							<li><a
								href="<%=basePath %>forumMore/${blogUser.id }.html?page=1">Homepage</a>
							</li>
							<c:if test="${page-1 >0}">
								<li><a
									href="<%=basePath %>forumMore/${blogUser.id }.html?page=${page-1}">Page up</a>
								</li>
							</c:if>
							<c:if test="${page-3 >0}">
								<li><a
									href="<%=basePath %>forumMore/${blogUser.id }.html?page=${page-3}">${page-3
										}</a></li>
							</c:if>
							<c:if test="${page-2 >0}">
								<li><a
									href="<%=basePath %>forumMore/${blogUser.id }.html?page=${page-2}">${page-2
										}</a></li>
							</c:if>
							<c:if test="${page-1 >0}">
								<li><a
									href="<%=basePath %>forumMore/${blogUser.id }.html?page=${page-1}">${page-1
										}</a></li>
							</c:if>
							...
							<c:if test="${page<lastPage}">
								<li><a
									href="<%=basePath %>forumMore/${blogUser.id }.html?page=${page+1}">Page down</a>
								</li>
							</c:if>
							<li><a
								href="<%=basePath %>forumMore/${blogUser.id }.html?page=${lastPage}">Last page</a>
							</li>
						</ul>
