  ...
  <div class="login_div" id="login_div">
	...
	<div class="blog_main">
		<div class="blog_title">
			...
			<button class="submitButton" onclick="javascrtpt:window.location.href='<%=basePath%>web/Blog/editAttach'"
			style="float: right;margin-top:-22px;margin-right:30px;">upload</button>
			</c:if>
		</div>
		<br>
				...
				<div class="attachContent">
					<table class="attachInfo" border="0" width="100%" cellspacing="0"
						...
						<tr class="tr1">
							...
							<th width="10%">download times</th>
						</tr>
						<!--13123213113-->
						<c:if test="${!empty attachByTime }">
						<c:forEach items="${attachByTime }" var="item" end="14">
							<tr>
				 ...
				 <c:if test="${!empty attachByTime }">
                       <div class="page">
						<ul>
							<li><a
								href="<%=basePath %>attachMore/${blogUser.id }.html?page=1">Homepage</a>
							</li>
							<c:if test="${page-1 >0}">
								<li><a
									href="<%=basePath %>attachMore/${blogUser.id }.html?page=${page-1}">Page up</a>
								</li>
							</c:if>
							<c:if test="${page-3 >0}">
								<li><a
									href="<%=basePath %>attachMore/${blogUser.id }.html?page=${page-3}">${page-3
										}</a></li>
							</c:if>
							<c:if test="${page-2 >0}">
								<li><a
									href="<%=basePath %>attachMore/${blogUser.id }.html?page=${page-2}">${page-2
										}</a></li>
							</c:if>
							<c:if test="${page-1 >0}">
								<li><a
									href="<%=basePath %>attachMore/${blogUser.id }.html?page=${page-1}">${page-1
										}</a></li>
							</c:if>
							...
							<c:if test="${page<lastPage}">
								<li><a
									href="<%=basePath %>attachMore/${blogUser.id }.html?page=${page+1}">Page down</a>
								</li>
							</c:if>
							<li><a
								href="<%=basePath %>attachMore/${blogUser.id }.html?page=${lastPage}">Last page</a>
							</li>
						</ul>
