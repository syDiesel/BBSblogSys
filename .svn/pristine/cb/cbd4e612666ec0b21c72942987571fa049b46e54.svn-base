			...
			<c:if test="${!empty photodt }">
               ...
               <div style="width:960px;overflow:auto;">
				...
				<div class="page">
					<ul>
						<li><a
							href="<%=basePath %>listAlbumPhoto.do?id=${album.id }&blId=${blogUser.id }&page=1">Homepage</a>
						</li>
						<c:if test="${page-1 >0}">
							<li><a
								href="<%=basePath %>listAlbumPhoto.do?id=${album.id }&blId=${blogUser.id }&page=${page-1}">Page up</a>
							</li>
						</c:if>
						<c:if test="${page-3 >0}">
							<li><a
						...
						<c:if test="${page<lastPage}">
							<li><a
								href="<%=basePath %>listAlbumPhoto.do?id=${album.id }&blId=${blogUser.id }&page=${page+1}">Page down</a>
							</li>
						</c:if>
						<li><a
							href="<%=basePath %>listAlbumPhoto.do?id=${album.id }&blId=${blogUser.id }&page=${lastPage}">Last page</a>
						</li>
					</ul>
