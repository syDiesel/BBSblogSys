...
<title>Gag User Management</title>
...
<div class="floor1" style="background:#fff;">
     ...
     <div class="manageForum">
          ...
          <div class="widget">
              ...
              <tr ><td>User Name</td><td>Level</td><td>Release Time</td><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td></tr>
             
             
              <c:if test="${listNotSayUser!=null}">
              <c:forEach items="${listNotSayUser}" var="listNotSayUser"> 
                  
                
              <tr>
              <td>
              <a  rel="nofollow" target="_blank" href="<%=basePath%>web/Blog/${listNotSayUser.nickName}">
              ${listNotSayUser.nickName}</a></td>
              <td>${listNotSayUser.userLevel}</td>
              <td>${listNotSayUser.isSay}</td>
              
              <td><a href="cancelNotSay_${masterBoardId}?notSayUser_id=${listNotSayUser.id}" 
              onclick="return confirm1()">Cancel</a></td>
              </tr>   
             ...
             <div class="fenye">
          	    <!-- 此处设置分页 -->
                         <c:if test="${record>1}">
						<a href="listNotSay_${masterBoardId}?record=${record-1}">Page up</a>
						</c:if>
						
					    <c:if test="${allPage<=10}">
						<c:forEach begin="1" end="${allPage}" step="1" var="i">
						      &nbsp; <a href="listNotSay_${masterBoardId}?record=${i}">
						...
						<a href="listNotSay_${masterBoardId}?record=${record+1}">Page down</a>
                         </c:if></div>
          </div>
