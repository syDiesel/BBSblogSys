...
<body style="font-family: arial">
	...
	<div class="mainbox">
		...
		<h3>${result }
		<a href="<%=basePath%>fileupload.do" style="text-decoration:underline;color: orange;">${result0 }</a>
		${result1 }
		<a href="<%=basePath%>fileupload.do" style="text-decoration: underline;color: orange;">${result2 }</a>
		</h3>
		<c:if test="${result=='Upload Photos successfully'||result=='Album already exists or create successfully'||result=='The original password is incorrect,change failed'||result=='Change the permissions of albums successfully' }">
		  <a href="<%=basePath%>fileupload.do">
   <button style="margin-top:20px;" class="submitButton">Continue to upload</button></a>
    <a href="<%=basePath%>listAlbum.do?id=${userInfo.id}">
   <button style="margin-top:20px;" class="submitButton">View the album</button></a>
		</c:if>
		  <div style="margin-top:20px;"><button	class="submitButton"
					style="float:right;width:60px;margin-top:80px;margin-right:150px;"
					onclick="location.href='javascript:history.go(-1);'">Back
							</button></div>
		</div>
	</div>
	<!--mainbox end-->
