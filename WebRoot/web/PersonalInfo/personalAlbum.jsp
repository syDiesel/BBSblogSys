...
<link href="<%=basePath%>css/newPersonalInfo/head.css" rel="stylesheet"
	type="text/css" />
<link href="<%=basePath%>css/newPersonalInfo/personalAlbum.css"
	...
	<div class="album_main">
		<div class="album_title">
		 ...
		 <c:if test="${userInfo.id eq blogUser.id }">
			...
			style="float: right;margin-top:-22px;">upload</a>
		 </c:if>
		</div>
		<br>
		<div class="album_content">
			...
			<c:if test="${clickPass != '1' }">
				<c:if test="${!empty albumdt }">
				   <div style="width:960px;overflow:auto;">
					<c:forEach begin="0" end="3" items="${albumdt }" var="item">
						<div class="album_img">
						<a href="<%=basePath %>listAlbumPhoto.do?id=${item.id}&blId=${blogUser.id}">
							<img src="<%=basePath %>${item.userAlbumPhoto.photo}" width=220px></a>
							<div class="album_name">${item.albumName}&nbsp;&nbsp;&nbsp;
							 <c:if test="${userInfo.id eq blogUser.id }">
							    <a href="<%=basePath%>deleteAlbum.do?id=${item.id}&page=${page }&blId=${blogUser.id}">delete</a>
							 </c:if>
							</div>
						</div>
					</c:forEach>
				...
				<c:if test="${empty albumdt }">
				    ...
				    <div class="noneAlbum">no album yet</div>
				</c:if>
				</c:if>
				<c:if test="${clickPass == '1' }">
					<div class="clickPass" id="clickPass_">
						<form action="<%=basePath %>confirmPass.html" method="post">
							<div class="click_pass">
								<span class="margin-top:15px;">input cipher:</span><input type="password" name="clickPass" value=""
							...
							<div class="click_butt">
								...
								&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <input
									type="reset" name="" value="reset" class="submitButton">
							</div>
						</form>
					</div>
				</c:if>
			...
			<c:if test="${!empty albumdt }">
					<div class="page">
						<ul>
							<li><a
								href="<%=basePath %>listAlbum.do?id=${blogUser.id }&page=1">Homepage</a>
							</li>
							<c:if test="${page-1 >0}">
								<li><a
									href="<%=basePath %>listAlbum.do?id=${blogUser.id }&page=${page-1}">Page up</a>
								</li>
							</c:if>
							<c:if test="${page-3 >0}">
								<li><a
									href="<%=basePath %>listAlbum.do?id=${blogUser.id }&page=${page-3}">${page-3
										}</a></li>
							</c:if>
							<c:if test="${page-2 >0}">
								<li><a
									href="<%=basePath %>listAlbum.do?id=${blogUser.id }&page=${page-2}">${page-2
										}</a></li>
							</c:if>
							<c:if test="${page-1 >0}">
								<li><a
									href="<%=basePath %>listAlbum.do?id=${blogUser.id }&page=${page-1}">${page-1
										}</a></li>
							</c:if>
							...
							<c:if test="${page<lastPage}">
								<li><a
									href="<%=basePath %>listAlbum.do?id=${blogUser.id }&page=${page+1}">Page down</a>
								</li>
							</c:if>
							<li><a
								href="<%=basePath %>listAlbum.do?id=${blogUser.id }&page=${lastPage}">Last page</a>
							</li>
						</ul>
