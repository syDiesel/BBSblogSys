		...
		<DIV class="con-l">
			...
			<DIV class="search-from2" style="position: relative;">
				...
				<DIV class="search-addition-con">
				      <c:if test="${value=='0'}">
                        <SPAN> <a
						href="<%=basePath%>blog.do?record=1&keyword=${keyword }&lab=${lab}&sub=${sub}&con=${con}" 
						style="font-size:14px;" class="submitButton_">relevancy rank</a>
					</SPAN><SPAN> &nbsp;&nbsp;&nbsp;<a
						href="<%=basePath%>orderByBlogTime.do?record=1&keyword=${keyword }&lab=${lab}&sub=${sub}&con=${con}"
						style="font-size:14px;color:#333;">time rank</a>
					</SPAN>
                    </c:if>
                    <c:if test="${value=='1'}">
                       <SPAN> <a
						href="<%=basePath%>blog.do?record=1&keyword=${keyword }&lab=${lab}&sub=${sub}&con=${con}"
						style="font-size:14px;color:#333;">relevancy rank</a>
					</SPAN><SPAN> &nbsp;&nbsp;&nbsp;<a
						href="<%=basePath%>orderByBlogTime.do?record=1&keyword=${keyword }&lab=${lab}&sub=${sub}&con=${con}" 
						style="font-size:14px;" class="submitButton_">time rank</a>
					</SPAN>
                    </c:if>
				</DIV>
			</DIV>
			...
			<c:if test="${!empty listBlogLog }">
				<c:forEach items="${listBlogLog }" var="listBlogLog">
					<DIV class="search-list-con">
						<DL class="search-list">
							<DT>
								<A href="<%=basePath%>web/Blog/article/<%=nowTime %>${listBlogLog.id}" target="_blank"><%=i++ %>、${listBlogLog.subject }</A>
							</DT>
								<DD class="author-time">
									author:<A href="<%=basePath %>web/Blog/${listBlogLog.blog.userInfo.nickName}"
										target="_blank">${listBlogLog.blog.userInfo.nickName }</A>&nbsp;&nbsp;&nbsp;kind: blog&nbsp;&nbsp;&nbsp;time:${listBlogLog.blogTime
									} &nbsp;&nbsp;&nbsp;read&nbsp;${listBlogLog.visited }&nbsp;post
								</DD>
								<DD class="search-detail">${listBlogLog.blogContent }</DD>
								<DD class="search-link">
									<A href="<%=basePath%>web/Blog/article/<%=nowTime %>${listBlogLog.id}"
										target="_blank"><%=basePath%>web/Blog/article/<%=nowTime %>${listBlogLog.id}</A>
								</DD>
						</DL>
					</DIV>
				</c:forEach>
       ...
       <!------------------------------------------------------分页--------------------------------------------------->
                          <c:choose>
                               <c:when test="${record>1 }">
                                   <c:if test="${value=='0'}"><a href="<%=basePath%>blog.do?record=1&keyword=<%=request.getAttribute("keyword")%>&lab=${lab}&sub=${sub}&con=${con}" 
                                   style="text-decoration: none;color:#0c87c9;">Homepage</a>&nbsp;&nbsp;</c:if>
                                   <c:if test="${value=='1'}"><a href="<%=basePath%>orderByBlogTime.do?record=1&keyword=<%=request.getAttribute("keyword")%>&lab=${lab}&sub=${sub}&con=${con}" 
                                   style="text-decoration: none;color:#0c87c9;">Homepage</a>&nbsp;&nbsp;</c:if>
                               </c:when>
                               <c:otherwise><a style="color:#0c87c9;">Homepage </a>&nbsp;&nbsp;</c:otherwise>
                          </c:choose>
                               
                          <c:choose>
                                <c:when test="${record>1 }">
                                   <c:if test="${value=='0'}"><a href="<%=basePath%>blog.do?record=${record-1}&keyword=<%=request.getAttribute("keyword")%>&lab=${lab}&sub=${sub}&con=${con}" 
                                   style="text-decoration: none;color:#0c87c9;">Page up</a>&nbsp;&nbsp;</c:if>
                                   <c:if test="${value=='1'}"><a href="<%=basePath%>orderByBlogTime.do?record=${record-1}&keyword=<%=request.getAttribute("keyword")%>&lab=${lab}&sub=${sub}&con=${con}" 
                                   style="text-decoration: none;color:#0c87c9;">Page up</a>&nbsp;&nbsp;</c:if>
                               </c:when>
                               <c:otherwise><a style="color:#0c87c9;">Page up</a>&nbsp;&nbsp;</c:otherwise>
                          </c:choose>
						  ...
						  <c:choose>
                                <c:when test="${record<allPage }">
                                   <c:if test="${value=='0'}"><a href="<%=basePath%>blog.do?record=${record+1}&keyword=<%=request.getAttribute("keyword")%>&lab=${lab}&sub=${sub}&con=${con}" 
                                   style="text-decoration: none;color:#0c87c9;">Page down</a>&nbsp;&nbsp;</c:if>
                                   <c:if test="${value=='1'}"><a href="<%=basePath%>orderByBlogTime.do?record=${record+1}&keyword=<%=request.getAttribute("keyword")%>&lab=${lab}&sub=${sub}&con=${con}" 
                                   style="text-decoration: none;color:#0c87c9;">Page down</a>&nbsp;&nbsp;</c:if>
                               </c:when>
                               <c:otherwise><a style="color:#0c87c9;">Page down</a>&nbsp;&nbsp;</c:otherwise>
                          </c:choose>
							
						  <c:choose>
                                <c:when test="${record<allPage }">
                                   <c:if test="${value=='0'}"><a href="<%=basePath%>blog.do?record=${allPage}&keyword=<%=request.getAttribute("keyword")%>&lab=${lab}&sub=${sub}&con=${con}" 
                                   style="text-decoration: none;color:#0c87c9;"> Last page  </a>&nbsp;&nbsp;</c:if>
                                   <c:if test="${value=='1'}"><a href="<%=basePath%>orderByBlogTime.do?record=${allPage}&keyword=<%=request.getAttribute("keyword")%>&lab=${lab}&sub=${sub}&con=${con}" 
                                   style="text-decoration: none;color:#0c87c9;"> Last page  </a>&nbsp;&nbsp;</c:if>
                               </c:when>
                               <c:otherwise><a style="color:#0c87c9;"> Last page </a>&nbsp;&nbsp;</c:otherwise>
                          </c:choose>
                          </ul>
				</DIV>
			</c:if>
