		...
		<DIV class="con-l">
			...
			<DIV class="search-from2" style="position: relative;">
				...
				<DIV class="search-addition-con">
				    <c:if test="${value=='0'}">
                        <SPAN> <a
						href="<%=basePath%>question.do?record=1&keyword=${keyword }&lab=${lab}&sub=${sub}&con=${con}" 
						class="submitButton_" style="font-size:14px;">relevancy rank</a>
					</SPAN><SPAN> &nbsp;&nbsp;&nbsp;<a
						href="<%=basePath%>orderByQuestionTime.do?record=1&keyword=${keyword }&lab=${lab}&sub=${sub}&con=${con}"
				...
				<c:forEach items="${listQuestion }" var="listQuestion">
					<DIV class="search-list-con">
						<DL class="search-list">
							<DT>
								<A href="<%=basePath %>web/QandA/QuestionFind/${listQuestion.id }" target="_blank"><%=i++%>、${listQuestion.getQSubject() }</A>
							</DT>
								<DD class="author-time">
									author:<A href="<%=basePath %>web/Blog/${listQuestion.userInfo.nickName}"
										target="_blank">${listQuestion.userInfo.nickName}</A>&nbsp;&nbsp;&nbsp;kind: Q&A&nbsp;&nbsp;&nbsp;time:${listQuestion.getQTime()
									} 
								</DD>
								<DD class="search-detail">${listQuestion.getQContent() }</DD>
								<DD class="search-link">
									<A href="<%=basePath %>web/QandA/QuestionFind/${listQuestion.id }"
										target="_blank"><%=basePath %>web/QandA/QuestionFind/${listQuestion.id }</A>
								</DD>
						</DL>
					</DIV>
				</c:forEach>
					...
					<ul class="fenye">
						...
						<c:choose>
							<c:when test="${record>1 }">
								<c:if test="${value=='0'}">
									<a
										href="<%=basePath%>question.do?record=1&keyword=<%=request.getAttribute("keyword")%>&lab=${lab}&sub=${sub}&con=${con}"
										style="text-decoration: none;color:#0c87c9;">Homepage
									</a>&nbsp;&nbsp;</c:if>
								<c:if test="${value=='1'}">
									<a
										href="<%=basePath%>orderByQuestionTime.do?record=1&keyword=<%=request.getAttribute("keyword")%>&lab=${lab}&sub=${sub}&con=${con}"
										style="text-decoration: none;color:#0c87c9;">Homepage
									</a>&nbsp;&nbsp;</c:if>
							</c:when>
							<c:otherwise>
						...
						<c:choose>
							<c:when test="${record>1 }">
								<c:if test="${value=='0'}">
									<a
										href="<%=basePath%>question.do?record=${record-1}&keyword=<%=request.getAttribute("keyword")%>&lab=${lab}&sub=${sub}&con=${con}"
										style="text-decoration: none;color:#0c87c9;">Page up
									</a>&nbsp;&nbsp;</c:if>
								<c:if test="${value=='1'}">
									<a
										href="<%=basePath%>orderByQuestionTime.do?record=${record-1}&keyword=<%=request.getAttribute("keyword")%>&lab=${lab}&sub=${sub}&con=${con}"
										style="text-decoration: none;color:#0c87c9;">Page up
									</a>&nbsp;&nbsp;</c:if>
							</c:when>
							<c:otherwise>
						...
						<c:choose>
							<c:when test="${record<allPage }">
								<c:if test="${value=='0'}">
									<a
										href="<%=basePath%>question.do?record=${allPage}&keyword=<%=request.getAttribute("keyword")%>&lab=${lab}&sub=${sub}&con=${con}"
										style="text-decoration: none;color:#0c87c9;">Last page
									</a>&nbsp;&nbsp;</c:if>
								<c:if test="${value=='1'}">
									<a
										href="<%=basePath%>orderByQuestionTime.do?record=${allPage}&keyword=<%=request.getAttribute("keyword")%>&lab=${lab}&sub=${sub}&con=${con}"
										style="text-decoration: none;color:#0c87c9;">Last page
									</a>&nbsp;&nbsp;</c:if>
							</c:when>
							<c:otherwise>
