		...
		<DIV class="con-l">
			...
			<DIV class="search-from2" style="position: relative;">
				...
				<DIV class="search-addition-con">
				      <c:if test="${value=='0'}">
                        <SPAN> <a
						href="<%=basePath%>bbs.do?record=1&keyword=${keyword }&lab=${lab}&sub=${sub}&con=${con}" 
						class="submitButton_" style="font-size:14px;">relevancy rank</a>
					</SPAN><SPAN> &nbsp;&nbsp;&nbsp;<a
						href="<%=basePath%>orderByForumTime.do?record=1&keyword=${keyword }&lab=${lab}&sub=${sub}&con=${con}"
				...
				<c:forEach items="${listForum }" var="listForum">
					<DIV class="search-list-con">
						<DL class="search-list">
							<DT>
								<A href="<%=basePath %>toTopics?post_id=${listForum.id }" target="_blank"><%=i++ %>、${listForum.subject }</A>
							</DT>
								<DD class="author-time">
									author:<A href="<%=basePath %>web/Blog/${listForum.userInfo.nickName }"
										target="_blank">${listForum.userInfo.nickName }</A>&nbsp;&nbsp;&nbsp;kind: post&nbsp;&nbsp;&nbsp;time:${listForum.postDate
									} &nbsp;&nbsp;&nbsp;reply&nbsp;${listForum.replyCount }&nbsp;post
								</DD>
								<DD class="search-detail">${listForum.postContent }</DD>
								<DD class="search-link">
									<A href="<%=basePath %>toTopics?post_id=${listForum.id }"
										target="_blank"><%=basePath %>toTopics?post_id=${listForum.id }</A>
								</DD>
						</DL>
					</DIV>
				</c:forEach>
       ...
       <!------------------------------------------------------分页--------------------------------------------------->
                          <c:choose>
                               <c:when test="${record>1 }">
                                   <c:if test="${value=='0'}"><a href="<%=basePath%>bbs.do?record=1&keyword=<%=request.getAttribute("keyword")%>&lab=${lab}&sub=${sub}&con=${con}" 
                                   style="text-decoration: none;color:#0c87c9;">Homepage </a>&nbsp;&nbsp;</c:if>
                                   <c:if test="${value=='1'}"><a href="<%=basePath%>orderByForumTime.do?record=1&keyword=<%=request.getAttribute("keyword")%>&lab=${lab}&sub=${sub}&con=${con}" 
                                   style="text-decoration: none;color:#0c87c9;">Homepage </a>&nbsp;&nbsp;</c:if>
                               </c:when>
                          ...
                          <c:choose>
                                <c:when test="${record>1 }">
                                   <c:if test="${value=='0'}"><a href="<%=basePath%>bbs.do?record=${record-1}&keyword=<%=request.getAttribute("keyword")%>&lab=${lab}&sub=${sub}&con=${con}" 
                                   style="text-decoration: none;color:#0c87c9;">Page up</a>&nbsp;&nbsp;</c:if>
                                   <c:if test="${value=='1'}"><a href="<%=basePath%>orderByForumTime.do?record=${record-1}&keyword=<%=request.getAttribute("keyword")%>&lab=${lab}&sub=${sub}&con=${con}" 
                                   style="text-decoration: none;color:#0c87c9;">Page up</a>&nbsp;&nbsp;</c:if>
                               </c:when>
						  ...
						  <c:choose>
                                <c:when test="${record<allPage }">
                                   <c:if test="${value=='0'}"><a href="<%=basePath%>bbs.do?record=${allPage}&keyword=<%=request.getAttribute("keyword")%>&lab=${lab}&sub=${sub}&con=${con}" 
                                   style="text-decoration: none;color:#0c87c9;">Last page </a>&nbsp;&nbsp;</c:if>
                                   <c:if test="${value=='1'}"><a href="<%=basePath%>orderByForumTime.do?record=${allPage}&keyword=<%=request.getAttribute("keyword")%>&lab=${lab}&sub=${sub}&con=${con}" 
                                   style="text-decoration: none;color:#0c87c9;">Last page </a>&nbsp;&nbsp;</c:if>
                               </c:when>
