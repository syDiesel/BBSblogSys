		...
		<DIV class="con-l">
			...
			<DIV class="search-from2" style="position: relative;">
				...
				<DIV class="search-addition-con">
				     <c:if test="${value=='0'}">
                        <SPAN> <a
						href="<%=basePath%>attachment.do?record=1&keyword=${keyword }&lab=${lab}&sub=${sub}&con=${con}" 
						class="submitButton_" style="font-size:14px;">
						relevancy rank</a>
					</SPAN><SPAN> &nbsp;&nbsp;&nbsp;<a
						href="<%=basePath%>orderByAttachTime.do?record=1&keyword=${keyword }&lab=${lab}&sub=${sub}&con=${con}"
				...
				<c:forEach items="${listAttach }" var="listAttach">
					<DIV class="search-list-con">
						<DL class="search-list">
							<DT>
								<A href="<%=basePath%>web/Blog/Attach/${listAttach.id }" target="_blank"><%=i++%>、${listAttach.attachName }</A>
							</DT>
								<DD class="author-time">
									author:<A href="<%=basePath %>web/Blog/${listAttach.userInfo.nickName}"
										target="_blank">${listAttach.userInfo.nickName}</A>&nbsp;&nbsp;&nbsp;kind: resource&nbsp;&nbsp;&nbsp;time:${listAttach.attachTime
									} &nbsp;&nbsp;&nbsp;download&nbsp;${listAttach.attachDownload }&nbsp;post
								</DD>
								<DD class="search-detail">${listAttach.attachDesc }</DD>
								<DD class="search-link">
									<A href="<%=basePath%>web/Blog/Attach/${listAttach.id }"
										target="_blank"><%=basePath%>web/Blog/Attach/${listAttach.id }</A>
								</DD>
						</DL>
					</DIV>
				</c:forEach>
					...
					<ul class="fenye">
						...
						<c:choose>
							<c:when test="${record>1 }">
								<c:if test="${value=='0'}">
									<a
										href="<%=basePath%>attachment.do?record=1&keyword=<%=request.getAttribute("keyword")%>&lab=${lab}&sub=${sub}&con=${con}"
										style="text-decoration: none;color:#0c87c9;">Homepage
									</a>&nbsp;&nbsp;</c:if>
								<c:if test="${value=='1'}">
									<a
										href="<%=basePath%>orderByAttachTime.do?record=1&keyword=<%=request.getAttribute("keyword")%>&lab=${lab}&sub=${sub}&con=${con}"
										style="text-decoration: none;color:#0c87c9;">Homepage
									</a>&nbsp;&nbsp;</c:if>
							</c:when>
							<c:otherwise>
						...
						<c:choose>
							<c:when test="${record>1 }">
								<c:if test="${value=='0'}">
									<a
										href="<%=basePath%>attachment.do?record=${record-1}&keyword=<%=request.getAttribute("keyword")%>&lab=${lab}&sub=${sub}&con=${con}"
										style="text-decoration: none;color:#0c87c9;">Page up
									</a>&nbsp;&nbsp;</c:if>
								<c:if test="${value=='1'}">
									<a
										href="<%=basePath%>orderByAttachTime.do?record=${record-1}&keyword=<%=request.getAttribute("keyword")%>&lab=${lab}&sub=${sub}&con=${con}"
										style="text-decoration: none;color:#0c87c9;">Page up
									</a>&nbsp;&nbsp;</c:if>
							</c:when>
							<c:otherwise>
						...
						<c:choose>
							<c:when test="${record<allPage }">
								<c:if test="${value=='0'}">
									<a
										href="<%=basePath%>attachment.do?record=${allPage}&keyword=<%=request.getAttribute("keyword")%>&lab=${lab}&sub=${sub}&con=${con}"
										style="text-decoration: none;color:#0c87c9;">Last page
									</a>&nbsp;&nbsp;</c:if>
								<c:if test="${value=='1'}">
									<a
										href="<%=basePath%>orderByAttachTime.do?record=${allPage}&keyword=<%=request.getAttribute("keyword")%>&lab=${lab}&sub=${sub}&con=${con}"
										style="text-decoration: none;color:#0c87c9;">Last page
									</a>&nbsp;&nbsp;</c:if>
							</c:when>
							<c:otherwise>
